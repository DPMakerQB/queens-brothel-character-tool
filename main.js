    ...
    if (canceled) {
        ...
        for (let outfit of psd.tree().children()) {
            if (outfit.get('name') !== "ignore") {
                outfits.push(outfit.get('name'))
            }
        }

        return {"filePath": filePaths[0], "outfits": outfits}
    }
}
...
async function build(_event, fp, chosenOutfitID) {
    fp = fp || "";
    chosenOutfitID = chosenOutfitID || "";
    ...
    for (let outfit of psd.tree().children()) {
        if (outfit.get('name') !== "ignore") {
            // If the user selected an outfit to export
            if (chosenOutfitID !== "") {
                if (outfit.get('name') !== chosenOutfitID) {
                    continue;
                }
            }
...
app.whenReady().then(() => {
    ...
    ipcMain.handle("openOutputFolder", openOutputFolder)
    createWindow()
